...
Configure an MCP client with that command and this repository as its working directory. The process communicates only through MCP JSON-RPC on stdin/stdout and exposes the seven tools documented below. It does not start Codex, Claude, or any execution runner -- including `adm_create_task` and `adm_global_invoke`, whose only side effect is creating a queued Task+Command through the existing Direct Dispatch ingress.

The stdio entrypoint uses the existing desktop OAuth service factory (already write-capable) for every tool, including `adm_create_task` and `adm_global_invoke`. The Cloud Run ASGI entrypoint uses application-default credentials and keeps the existing split between a read-only service account (`adm_dispatch`/`adm_status`/`adm_runtime_quota_status`) and a write-scoped one (`adm_create_task`, `adm_global_invoke`, mirroring `cloud/app.py`'s existing `default_service_factory`/`default_write_service_factory` split for the REST routes) plus the same GCS bucket the REST Direct Dispatch route needs for idempotency. As of this writing that write scope and bucket are not yet provisioned on the live `adm-runtime-bridge` service -- see the deployment plan for the exact infra delta. `adm_global_invoke`'s server-side baseline resolution additionally needs a real canonical git checkout reachable from wherever it runs (present on the local Windows stdio transport, matching `manager/worktree_materializer.py`'s environment) -- the Cloud Run ASGI transport has no such checkout provisioned yet, a separate, not-yet-addressed infra gap from the Drive write scope/bucket one above.
...
- `adm_global_invoke` (write, idempotent): the Global Hands-off Execution Layer's stable, project-identity/alias-driven entry point (`manager.global_invoke`). Accepts `project` (canonical project_id or a registered alias), `title`, `goal`, `idempotency_key`, an optional `repo_write` flag, `allowed_paths` (required iff `repo_write`), and optional `preferred_provider`/`account_id`. There is no field for repo, baseline commit, or a governance/PROJECT-RULES reference -- `project` is resolved deterministically through the Global Project Registry (`manager/project_registry.py`, fail-closed on an ambiguous alias or unknown/disabled project), repo identity is cross-checked between the registry and the Drive project record, baseline_head is resolved server-side from the canonical checkout against the project's own `baseline_resolution_policy`, and governance/PROJECT-RULES references are verified present before `repo_write: true` is ever admitted. It then calls the same authenticated Direct Dispatch ingress `adm_create_task` uses (`cloud.dispatch_ingress.handle_dispatch`), never a provider or execution runner directly, and stamps the resolved governance/PROJECT-RULES/repo evidence onto the created Task's `source_context`. Replaying the same `idempotency_key` returns the identity already created.

MCP SDK 2.0's generated function schema does not emit `additionalProperties: false`; unmodeled fields are ignored by the SDK and are not forwarded to the quota loader, `adm_create_task`, or `adm_global_invoke` -- a caller cannot smuggle `provider`, `account_id`, `executable`, `env`, `repo`, `baseline_head`, or any other field through the tool call.

All tools except `adm_create_task` and `adm_global_invoke` declare MCP `readOnlyHint=true`; those two declare `readOnlyHint=false, idempotentHint=true`. All tools declare `destructiveHint=false`. Read-only bridge calls all use `read_only=True`; there are no update, completion, handoff, or AI-launch tools -- `adm_create_task` and `adm_global_invoke` are the only create surfaces, and both only ever produce a `queued` Command for the existing, unmodified Command Watcher pipeline to pick up under its own Safe Auto-Admission and allowlist rules.

...
class DriveDispatchIngressTests(unittest.TestCase):
    ...
    def test_abbreviated_repo_write_baseline_expands_from_activated_sha(self):
        full_baseline = "80138e1" + "a" * 33
        short_request = {
            **self.VALID_REPO_WRITE,
            "baseline_head": "80138e1",
        }
        service = Service(request(repo_write=short_request))
        handler = Mock(return_value={"accepted": True, "request_id": "drive-e2e-1",
                                     "task_id": "dispatch-drive-e2e-1",
                                     "command_id": "dispatch-drive-e2e-1", "status": "queued"})

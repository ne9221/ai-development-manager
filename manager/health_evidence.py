...
probe or act. Dashboard is the one component this task adds a live
recovery path for (see desktop/AdmCommon.ps1's
Confirm-AdmDashboardAlive); its evidence is recorded here via `record()`
after PowerShell has already made the real HTTP/process observation.
...
# module is meant to be cheap to invoke on every tray poll tick, so it
# keeps its own copy (identical behavior: temp file + os.replace) instead
# of taking on that dependency weight.

...
Authentication uses, in order: an existing (or refreshed) token at
`GOOGLE_DRIVE_TOKEN`, Google Application Default Credentials, an official
Desktop OAuth client JSON selected with `GOOGLE_OAUTH_CLIENT_SECRETS` (if
set), or otherwise the ADM bundled default Desktop OAuth client
(`manager/default_oauth_config.py`). Credentials remain outside the
repository.
...
| Antigravity | Official IDE language server, Connect-RPC `RetrieveUserQuotaSummary` + `GetUserStatus` over loopback HTTP (`x-codeium-csrf-token`) | Yes | Verified 2026-09-02 (`collectors/antigravity.py`, discovery in `manager/ag_language_server.py`): two model groups x (5h + weekly) buckets with `remainingFraction`/`resetTime`, plus account email/plan and per-model quota. Requires the IDE (or its persistent language server) to be running; fails closed to the last-good entry otherwise. No `agy` binary exists on this machine. |
